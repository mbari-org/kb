...
const useTaxonomyIntegrity = () => {
  const checkIntegrity = useCallback(taxonomy => {
      ...
      if (name === rootName) {
        ...
        if (typeof concept.parent !== 'string' || concept.parent.length === 0) {
          ...
          if (!Array.isArray(parent.children)) {
            addError(`Parent "${parent.name}" has invalid children; expected an array`)
          } else if (!parent.children.includes(name)) {
    ...
    Object.keys(aliasMap).forEach(alias => {
      ...
      if (nameConflict && nameConflict !== owner) {
        addError(
          `alias "${alias}" conflicts with primary name of another concept "${nameConflict.name}"`
        )
      }
    })
