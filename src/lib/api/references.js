...
const addReferenceConcept = async (config, [referenceId, conceptName]) =>
...
const deleteReference = async (config, referenceId) => oniDelete({ config, path: ['references', referenceId] })
...
const removeReferenceConcept = async (config, [referenceId, conceptName]) =>
  oniPut({ config, path: ['references', 'remove', referenceId, 'from', conceptName] })

const renameReferenceConcept = async (config, [referenceId, oldName, newName]) => {
  await removeReferenceConcept(config, [referenceId, oldName])
  return addReferenceConcept(config, [referenceId, newName])
}
...
export {
  ...
  getReference,
  getReferences,
  removeReferenceConcept,
  renameReferenceConcept,
  updateReference,
}
