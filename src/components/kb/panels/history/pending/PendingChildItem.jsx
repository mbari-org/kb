...
const PendingChildItem = ({ item }) => {
  const title = pendingActionValue(item)
  const childSx = otherApprovalSx(APPROVAL.ACCEPT)

  const pendingDetailTitle = (
    <Box>
      <PendingItemTitle sx={childSx} action={item.action} title={title} />
    </Box>
  )
...
export default PendingChildItem
